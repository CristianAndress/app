...
def activate():
    try:
        ...
        if request.method == "GET": #
            number = request.args['auth'] #ESTO ES PARA GET 
...
def confirm():
    try:
        ...
        if request.method == "POST": 
            ...
            if attempt is not None:
                db.execute(
                    "Update forgotlin Set state = ? Where id = ?", (utils.F_INACTIVE, attempt['id'])
                )
                salt = hex(random.getrandbits(128))[2:]
                hashP = generate_password_hash(password + salt)   
                db.execute(
...
def login():
    try:
        ...
        if request.method == "POST":
            ...
            if error is None:
                session.clear()
                session['user_id'] = user["id"]#### guarda el dato user_id
                return redirect(url_for('inbox.show'))
...
def load_logged_in_user():
    user_id = session.get("user_id")##### carga el dato user_id

    if user_id is None:
        g.user = None
    else:
        g.user = get_db().execute(
            "Select * from user where id = ?  ", (user_id,) #obtiene el usuario que esta logueado
        ).fetchone()
